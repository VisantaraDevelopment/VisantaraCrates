...
import me.bintanq.visantaracrates.util.PhysicalKeyItem;
import me.bintanq.visantaracrates.util.UUIDResolver;
...
public class KeyManager {
    ...
    private final Map<String, me.bintanq.visantaracrates.model.KeyDefinition> keysDef = new HashMap<>();

    public KeyManager(VisantaraCrates plugin, PlayerDataManager playerDataManager) {
        this.plugin = plugin;
    ...
    public boolean giveKey(Player player, String keyId, int amount) {
        if (globalMode == KeyMode.VIRTUAL) {
            ...
            while (remaining > 0) {
                ...
                ItemStack key = buildPhysicalKeyItem(keyId, stack);
    ...
    public boolean hasRequiredKeys(Player player, Crate crate) {
        if (crate.getRequiredKeys() == null || crate.getRequiredKeys().isEmpty()) return true;
        if (crate.getKeyRequirementMode() == Crate.KeyRequirementMode.ANY) {
            return crate.getRequiredKeys().stream()
                    .anyMatch(req -> countAvailable(player, req) >= req.getAmount());
        } else {
            return crate.getRequiredKeys().stream()
    ...
    public int countPossibleOpens(Player player, Crate crate) {
        if (crate.getRequiredKeys() == null || crate.getRequiredKeys().isEmpty()) return 9999;
        if (crate.getKeyRequirementMode() == Crate.KeyRequirementMode.ANY) {
            return crate.getRequiredKeys().stream()
    ...
    private int countAvailable(Player player, Crate.KeyRequirement req) {
        return switch (req.getType()) {
            ...
            case NEXO       -> { var h = plugin.getHookManager().getNexoHook();       yield h != null ? h.countKey(player, req.getKeyId()) : 0; }
        };
    }

    public String consumeKeys(Player player, Crate crate) {
        if (crate.getKeyRequirementMode() == Crate.KeyRequirementMode.ANY) {
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                if (countAvailable(player, req) >= req.getAmount()) {
            ...
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                if (countAvailable(player, req) < req.getAmount()) return null;
            }
            String firstKeyId = null;
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                if (firstKeyId == null) firstKeyId = req.getKeyId();
                if (!consumeKey(player, req)) {
                    Logger.warn("Key consume failed mid-way: " + player.getName() + " key=" + req.getKeyId());
                    return null;
                }
            }
            return firstKeyId != null ? firstKeyId : "";
        }
    }
    ...
    private boolean consumeKey(Player player, Crate.KeyRequirement req) {
        return switch (req.getType()) {
            ...
            case NEXO       -> { var h = plugin.getHookManager().getNexoHook();       yield h != null && h.removeKey(player, req.getKeyId(), req.getAmount()); }
        };
    }
     ...
     */
    public String consumeKeysBatch(Player player, Crate crate, int count) {
        if (crate.getKeyRequirementMode() == Crate.KeyRequirementMode.ANY) {
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                int totalNeeded = req.getAmount() * count;
                if (countAvailable(player, req) >= totalNeeded) {
            ...
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                int totalNeeded = req.getAmount() * count;
                if (countAvailable(player, req) < totalNeeded) return null;
            }
            String firstKeyId = null;
            for (Crate.KeyRequirement req : crate.getRequiredKeys()) {
                if (firstKeyId == null) firstKeyId = req.getKeyId();
    ...
    private boolean consumeKeyAmount(Player player, Crate.KeyRequirement req, int totalAmount) {
        return switch (req.getType()) {
            ...
            case NEXO       -> { var h = plugin.getHookManager().getNexoHook();       yield h != null && h.removeKey(player, req.getKeyId(), totalAmount); }
        };
    }
    ...
    private void loadKeysConfig() {
        ...
        org.bukkit.configuration.ConfigurationSection section = config.getConfigurationSection("keys");
        if (section == null) return;

        for (String keyId : section.getKeys(false)) {
            org.bukkit.configuration.ConfigurationSection keySec = section.getConfigurationSection(keyId);
            if (keySec == null) continue;

            String displayName = keySec.getString("display-name", keyId);
            String material = keySec.getString("material", "TRIPWIRE_HOOK");
            int customModelData = keySec.getInt("custom-model-data", -1);
            List<String> lore = keySec.getStringList("lore");
            String nexoItem = keySec.getString("nexo-item", null);

            me.bintanq.visantaracrates.model.KeyDefinition def = new me.bintanq.visantaracrates.model.KeyDefinition(
                    keyId, displayName, material, customModelData, lore, nexoItem);
            keysDef.put(keyId.toLowerCase(), def);
        }
        Logger.info("Loaded &e" + keysDef.size() + " &fkey definitions from keys.yml.");
    }

    public me.bintanq.visantaracrates.model.KeyDefinition getKeyDefinition(String keyId) {
        if (keyId == null) return null;
        return keysDef.get(keyId.toLowerCase());
    }

    public ItemStack buildPhysicalKeyItem(String keyId, int amount) {
        me.bintanq.visantaracrates.model.KeyDefinition def = getKeyDefinition(keyId);
        if (def != null) {
            if (def.hasNexoItem() && plugin.isNexoEnabled()) {
                var nexoHook = plugin.getHookManager().getNexoHook();
                if (nexoHook != null) {
                    ItemStack nexoItem = nexoHook.buildItemById(def.getNexoItem());
                    if (nexoItem != null) {
                        nexoItem.setAmount(Math.max(1, amount));
                        ItemMeta meta = nexoItem.getItemMeta();
                        if (meta != null) {
                            if (def.getDisplayName() != null && !def.getDisplayName().isEmpty()) {
                                meta.setDisplayName(colorize(def.getDisplayName()));
                            }
                            if (def.getLore() != null && !def.getLore().isEmpty()) {
                                List<String> coloredLore = new ArrayList<>();
                                def.getLore().forEach(l -> coloredLore.add(colorize(l)));
                                meta.setLore(coloredLore);
                            }
                            org.bukkit.NamespacedKey nsKey = new org.bukkit.NamespacedKey(plugin, "qc_key_id");
                            meta.getPersistentDataContainer().set(nsKey, org.bukkit.persistence.PersistentDataType.STRING, keyId);
            ...
            Material mat = Material.matchMaterial(def.getMaterial());
            if (mat == null) mat = defaultPhysicalMaterial;

            return PhysicalKeyItem.create(
                    plugin,
                    keyId,
                    amount,
                    def.getDisplayName(),
