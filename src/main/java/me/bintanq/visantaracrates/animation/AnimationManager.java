...
import me.bintanq.visantaracrates.animation.impl.*;
...
public class AnimationManager {
    ...
    public AnimationManager(VisantaraCrates plugin) {
        this.plugin = plugin;
        ...
        animationCache.put(Crate.GuiAnimationType.CHOICE,      new ChoiceAnimation(plugin));
    }
    ...
    public void startAnimation(Player player, Crate crate, RewardResult result, String consumedKeyId) {
        ...
        if (consumedKeyId != null) {
            session.getMetadata().put("consumedKeyId", consumedKeyId);
        }
        sessions.put(player.getUniqueId(), session);
        session.setRunning(true);
        resolveAnimation(crate, consumedKeyId).start(session);
    }

    public void registerSession(CrateSession session) {
